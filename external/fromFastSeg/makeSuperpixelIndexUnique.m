...
function [ output, superpixelFrameIDs, frameSuperpixelBounds, globalIDs ] = ...
    ...
    for( frame = 1: length( superpixels ) )
        ...
        indexes = uint32( max(max( superpixels{ frame } ) ));
        output{ frame } = globalIDs + uint32( superpixels{ frame } );
    ...
    for( frame = 1: length( superpixels ) )
        superpixelFrameIDs( frameSuperpixelBounds( frame ): ...
            frameSuperpixelBounds( frame + 1 ) - 1 ) = frame;
    end
